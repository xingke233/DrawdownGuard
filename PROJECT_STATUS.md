...
- 可在每次每日检查后写入 `daily_log.json`，并支持同日同基金更新。
- 可通过 `scripts/run_daily.sh` 自动运行每日检查，并将终端输出保存到 `logs/run_YYYY-MM-DD.txt`。
- 可按配置发送每日邮件检查报告，邮件失败不会中断主流程。
- 可运行历史回测并输出 `backtest_report.json`。
- 可基于补仓事件估算策略模拟收益，包括份额、市值、浮盈和总收益率。
- 可运行资产级回测并输出 `asset_backtest_report.json`，用于与基金级回测对照。
- 可运行组合级定投加补仓回测并输出 `portfolio_backtest_report.json`。
- 可基于回测报告生成 PNG 可视化图表，输出到 `backtest_plots/`。
...
- `python3 main.py portfolio-backtest`：运行组合级定投加补仓回测。
- `python3 main.py portfolio-report`：查看最近一次组合回测摘要。
- `python3 main.py weekly-dca`：分析周一到周五不同定投日的组合回测表现。
- `python3 main.py strategy-lab`：比较不同回撤补仓档位的组合回测表现。
- `python3 main.py strategy-lab-report`：查看 Strategy Lab 收益率、现金效率和风险排名。
- `python3 main.py backtest-scenarios`：运行多参数回测场景并输出 `scenarios_report.json`，同时打印场景摘要。
- `python3 main.py scenarios-return`：查看多参数场景收益估算。
- `python3 main.py backtest-plot`：读取 `backtest_report.json` 或 `scenarios_report.json` 生成回测可视化图表。
...
- `logs/run_YYYY-MM-DD.txt` 保存脚本运行输出；`logs/*.txt` 已加入 `.gitignore`，仅保留 `logs/.gitkeep`。
...
- `asset_backtest_report.json` 输出资产级触发次数、收益率和现金消耗。
...
- `portfolio_backtest_report.json` 输出组合总投入、定投投入、补仓投入、组合估算市值、浮盈亏、总收益率、子弹仓剩余、资产贡献和 skipped 资产。
- `weekly_dca_analysis.json` 输出周一到周五定投日对组合总投入、组合市值、浮盈亏、总收益率、子弹仓剩余和补仓触发次数的影响。
- `strategy_lab_report.json` 输出 A/B/C/D 四组回撤补仓档位的收益率、浮盈、市值、触发次数、子弹仓剩余和最大回撤，并给出收益率、现金效率和风险排名。
- 多参数回测场景默认覆盖 `initial_cash` 为 2000/3000/5000，`monthly_cash_addition` 为 0/200/500 的组合。
- `scenarios_report.json` 包含 `summary.scenarios` 和 `summary.fund_comparisons`，用于比较子弹仓使用、剩余现金、触发频率和策略模拟收益。
- V2.2 可视化输出按 scenario 分目录保存 PNG，例如 `backtest_plots/S001/270042_..._S001.png`。
